...
- Evita lenguaje de informe automático y frases clónicas repetidas.
...
- "status" debe ser una frase descriptiva (minimo 12 palabras) sobre el clima y la direccion del debate. Nunca uses una palabra suelta como "completo", "activo" o "cerrado".
- Responde en espanol.
...
- En cada punto clave, prioriza conflicto, argumentos y giros del hilo; evita frases genéricas.
- En cada participante, resume postura y por qué destaca (actividad, votos o impacto en la discusión).
- "status" debe ser una frase descriptiva (minimo 12 palabras) sobre el clima final del debate. Nunca uses una palabra suelta como "completo", "activo" o "cerrado".
- Responde en espanol.
...
SALIDA:
{
  ...
  "participants": [
    { "name": "Usuario1", "contribution": "2-3 frases breves: postura concreta, a qué responde y por qué destaca. Termina con punto." },
    "... hasta ${maxParticipants}"
  ],
  "status": "Frase ORIGINAL de 18-40 palabras sobre el clima del debate. Ejemplo: 'El clima es de alta tensión y fragmentación, con el foro partido en bandos personales y un pesimismo generalizado sobre el futuro del club.'"
}

REGLAS CRITICAS (cumple TODAS):
- SOLO JSON, sin markdown ni texto extra. Empieza con "{" y termina con "}".
- DETALLE: Cada "contribution" en 2-3 frases breves (aprox. 20-55 palabras). Cada punto clave en 1-3 frases breves. Distribuye el espacio EQUITATIVAMENTE entre TODOS los participantes y evita párrafos largos.
- Cada "contribution" y cada punto clave DEBE terminar con punto (.).
- PROHIBIDO usar frases genéricas como "participó activamente en el debate", "aportando argumentos y réplica" o "con presencia constante". Si no tienes información concreta sobre un usuario, NO lo incluyas.
...
- El "status" DEBE ser una frase ORIGINAL de 18-40 palabras que TU redactes sobre el clima del debate. NUNCA escribas una sola palabra como "activo", "open" o "cerrado".

REGLAS DE CONTENIDO:
- Resume todo el bloque con visión global. Ignora posts vacíos ("pole", "+1"). Incluye spoilers relevantes.
- Escribe con tono natural y humano. Refleja ironía, tensión, consenso o cachondeo cuando aporte contexto.
...
SALIDA:
{
  ...
  "status": "Frase ORIGINAL de 18-40 palabras sobre el clima final del debate. Ejemplo: 'El debate se cierra con un pesimismo generalizado y una fractura total entre facciones que priorizan a sus jugadores favoritos por encima del bien colectivo.'"
}
...
- Cada "contribution" y cada punto clave DEBE terminar con punto (.).
- PROHIBIDO usar frases genéricas como "participó activamente en el debate" o "aportando argumentos y réplica". Si no tienes información concreta, NO incluyas al usuario.
...
- El "status" DEBE ser una frase ORIGINAL de 18-40 palabras. NUNCA una sola palabra como "activo", "open" o "cerrado".
