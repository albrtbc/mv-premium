...
/**
 * Builds a summary prompt parameterized by type and page count.
 */
export function buildSummaryPrompt({ type, pageCount }: PromptConfig): string {
...
export function buildSingleBatchPromptGemini(pageCount: number): string {
	return buildSummaryPrompt({ type: 'batch', pageCount })
}
export function buildMetaSummaryPromptGemini(pageCount: number): string {
	return buildSummaryPrompt({ type: 'meta', pageCount })
}
